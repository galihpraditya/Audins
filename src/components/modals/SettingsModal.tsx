  ...
  return (
    ...
    >
        ...
        <div className="flex border-b border-border bg-surface-2/60 px-6 gap-2 pt-2">
          {[
            { id: "api", label: t("tab_api_key"), icon: <Key size={14} weight="duotone" /> },
            { id: "portfolio", label: t("tab_about"), icon: <Cpu size={14} weight="duotone" /> },
          ].map((tab) => (
            <button
              key={tab.id}
          ...
          {activeTab === "api" && (
            <div className="space-y-5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-fg-secondary leading-relaxed">
                      ...
                      <span className="font-mono text-fg font-bold">
                        {Math.max(0, maxUploads - uploadCount)} of {maxUploads} {t("uploads_left")}
                      </span>
                      ).
                    </p>
                  </div>
              ...
              <div className="space-y-2">
                <label htmlFor="settings-api-key" className="block text-xs font-mono font-semibold text-fg-secondary uppercase tracking-wider">
            ...
            <div className="space-y-5 text-xs text-fg-secondary leading-relaxed">
              <div className="p-5 rounded-2xl bg-surface-2/80 border border-border space-y-2">
                ...
                <p className="text-xs text-fg-secondary">
                  High-speed audio intelligence platform for speech-to-text transcription and structured executive summaries.
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="p-3 rounded-xl bg-surface-2 border border-border flex items-center gap-2">
                    <Lightning size={16} weight="duotone" className="text-fg-tertiary" />
                    <div>
                      <p className="font-semibold text-fg">Whisper LPU</p>
                      <p className="text-[10px] font-mono text-fg-tertiary">Speech-to-Text</p>
                    </div>
                  </div>
                  <div className="p-3 rounded-xl bg-surface-2 border border-border flex items-center gap-2">
                    <Cpu size={16} weight="duotone" className="text-fg-tertiary" />
                    <div>
                      <p className="font-semibold text-fg">Llama 3.3 70B</p>
                      <p className="text-[10px] font-mono text-fg-tertiary">Summarization</p>
                    </div>
                  </div>
                  <div className="p-3 rounded-xl bg-surface-2 border border-border flex items-center gap-2">
                    <HardDrives size={16} weight="duotone" className="text-fg-tertiary" />
                    <div>
                      <p className="font-semibold text-fg">Cloudflare R2</p>
                      <p className="text-[10px] font-mono text-fg-tertiary">Audio Storage</p>
                    </div>
                  </div>
                  <div className="p-3 rounded-xl bg-surface-2 border border-border flex items-center gap-2">
                    <Globe size={16} weight="duotone" className="text-fg-tertiary" />
                    <div>
                      <p className="font-semibold text-fg">React 19 + Vite</p>
                      <p className="text-[10px] font-mono text-fg-tertiary">Frontend Studio</p>
                    </div>
                  </div>
                </div>
              </div>
