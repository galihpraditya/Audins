...
export default function TranscriptPanel({ entries, currentTime, onSeekTo }: TranscriptPanelProps) {
  ...
  return (
    <div className="flex-1 overflow-y-auto flex flex-col bg-surface" ref={containerRef}>
      <div className="px-4 py-3 sticky top-0 border-b border-border bg-surface z-10 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-mono text-fg-tertiary">Click to seek</span>
        </div>
        ...
        <div className="relative group">
          ...
          />
          <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-3.5 h-3.5 text-fg-tertiary absolute left-2.5 top-1/2 -translate-y-1/2">
          ...
          {searchQuery && (
            ...
            >
              <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-3 h-3">
                <path strokeLinecap="round" strokeLinejoin="round" d="M6 14l8-8m-8 0l8 8" />
              </svg>
            </button>
          )}
        </div>
      ...
      <div className="p-3 space-y-1">
        {filteredEntries.length === 0 ? (
          <p className="text-xs text-center text-fg-tertiary mt-4">No results found for "{searchQuery}"</p>
        ) : (
          filteredEntries.map((entry, index) => {
            const isActive = index === (activeEntryIndex >= 0 ? activeEntryIndex : 0) && !searchQuery
            return (
              <button
                key={entry.ts + index}
              ...
              >
                <div className="flex items-start gap-2.5">
                  ...
                  <p className="text-xs leading-relaxed">{highlightText(entry.text, searchQuery)}</p>
                </div>
              </button>
            )
          })
