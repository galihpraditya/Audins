...
export default function App() {
  ...
  const [liveRecorderMinimized, setLiveRecorderMinimized] =
  ...
  const isQuotaError = (err: unknown): boolean => {
    if (err instanceof ApiError) {
      if (err.status === 429 || err.status === 402 || err.status === 503)
        return true
    }
  ...
  const { quota, refreshFromServer, bumpUploadCount, rollbackUploadCount } =
    useQuota()
    ...
    (
      ...
      if (outcome.type === "finished") {
        ...
        if (updated.status === "Completed") {
          showToast(
            t("toast_processing_done", { name: updated.name }),
            "success",
          )
          ...
          showToast(
            t("toast_processing_failed", { name: updated.name }),
            "error",
          )
        }
      } else {
  ...
  const { start: startPolling, stop: stopPolling } =
    useDocumentPolling(handlePollOutcome)
  ...
  const handleUploadFile = async (file: File, knownDurationSec?: number) => {
    ...
    try {
      ...
      if (!hasCustomKey && isQuotaError(err)) {
        ...
        showToast(
          `Processing failed: ${(err as Error).message || "Unknown error"}`,
          "error",
        )
      }
    }
  ...
  const handleDeleteDocument = async (id: number | string) => {
    ...
    try {
      ...
      showToast(
        t("toast_delete_failed", { error: (err as Error).message }),
        "error",
      )
    }
  }
  ...
  const handleRenameDocument = async (id: number | string, newName: string) => {
    try {
      ...
      showToast(
        t("toast_rename_failed", { error: (err as Error).message }),
        "error",
      )
    }
  }
  ...
  const handleDuplicateDocument = async (doc: DocumentItem) => {
    try {
      ...
      showToast(
        t("toast_duplicate_failed", { error: (err as Error).message }),
        "error",
      )
    }
  }
  ...
  const handleUpdateSummary = async (
    ...
    try {
      ...
      showToast(
        t("toast_summary_save_failed", { error: (err as Error).message }),
        "error",
      )
    }
  }
  ...
  const handleUpdateDocument = useCallback((updated: DocumentItem) => {
  ...
  if (isPublicSharedRoute) {
    return (
      <Routes>
        <Route path="/s/:shareId" element={<SharedNotePage />} />
