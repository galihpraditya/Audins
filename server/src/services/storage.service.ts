import { FullDocument, DocumentItem, TranscriptEntry, AISummary } from '../types/index.js'
...
const INITIAL_DOCS: FullDocument[] = [
  {
    ...
    transcripts: [
      { ts: '0:00:12', seconds: 12, text: 'Good morning everyone, thanks for joining the Q3 earnings call. We have a strong quarter to report today.' },
      { ts: '0:00:31', seconds: 31, text: 'Revenue came in at $42.7 million, representing 34% year-over-year growth, exceeding our guidance of $40 million.' },
      { ts: '0:01:04', seconds: 64, text: 'Gross margins expanded by 210 basis points to 68.4%, driven primarily by infrastructure cost optimizations.' },
      { ts: '0:01:38', seconds: 98, text: 'Our enterprise customer count grew to 312, up from 241 in Q2. Average contract value increased 18% to $136k annually.' },
      { ts: '0:02:11', seconds: 131, text: 'Net revenue retention stands at 127%, reflecting strong expansion within existing accounts.' },
      { ts: '0:02:44', seconds: 164, text: 'Looking at regional performance — North America contributed 61% of total revenue, EMEA 24%, and APAC 15%.' },
      { ts: '0:03:19', seconds: 199, text: 'We closed 3 landmark deals this quarter including a $4.2M TCV contract with a Fortune 100 financial services firm.' },
      { ts: '0:03:52', seconds: 232, text: 'R&D investment increased to 22% of revenue as we accelerate our AI roadmap and platform capabilities.' },
      { ts: '0:04:28', seconds: 268, text: 'Headcount grew from 487 to 531 employees, with a focus on engineering and customer success roles.' },
      { ts: '0:05:01', seconds: 301, text: 'For Q4, we are guiding to revenue of $46–48 million and expect to exit the year with positive operating cash flow.' },
      { ts: '0:05:34', seconds: 334, text: "I'll now turn it over to Sarah for a deeper dive on our product milestones and what's coming in Q4." },
      { ts: '0:05:58', seconds: 358, text: 'Thanks, Michael. This quarter we shipped 14 major product updates including our new AI-powered analytics suite.' },
    ],
    summary: {
  ...
  },
  {
    ...
    transcripts: [
      { ts: '0:00:05', seconds: 5, text: 'Welcome team. Today we review the H2 product roadmap priorities and feature allocations.' },
    ],
  },
  {
    id: 'doc-3',
...
export async function renameDocument(id: string, newName: string): Promise<FullDocument | null> {
...
export async function duplicateDocument(id: string): Promise<FullDocument | null> {
  const doc = await getDocumentById(id)
  if (!doc) return null
