...
async function readRecord(key: string): Promise<IPRecord | null> {
  if (isSupabaseRateLimitEnabled()) {
    ...
    if (remote !== null) return { count: remote.count, resetTime: remote.resetTime }
...
export async function checkPortfolioRateLimit(
  ...
  try {
    ...
    if (isSupabaseRateLimitEnabled()) {
      const incremented = await incrementSupabaseRateLimit(key, effectiveMaxLimit)
    ...
    if (!record || now > record.resetTime) {
      record = { count: 0, resetTime: new Date(now.getTime() + ROLLING_WINDOW_MS) }
    }
